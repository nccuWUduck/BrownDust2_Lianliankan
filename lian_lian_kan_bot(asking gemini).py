...
# 掃描遊戲棋盤的間隔時間（秒）
SCAN_INTERVAL = 0.2 # 可以嘗試提高到 0.5 或 1.0 如果依然當機
...
def is_nearly_solid_color(cv2_img: np.ndarray, tolerance: int) -> Tuple[bool, Optional[np.ndarray], List[np.ndarray]]:
    ...
    for r_idx in range(3):
        for c_idx in range(3):
            y_start = r_idx * sub_h
            y_end = y_start + sub_h
            x_start = c_idx * sub_w # <--- 修正：c_start 應該是 c_idx * sub_w
            x_end = x_start + sub_w # <--- 修正：x_end 應該是 x_start + sub_w
...
class LianLianKanBot:
    ...
    def _get_game_board_tiles(self, game_window_img: np.ndarray) -> Tuple[List[List[str]], Dict[Tuple[int, int], Tuple[int, int]], np.ndarray]:
        ...
        # tile_centers 只儲存可點擊的實體方塊的中心絕對座標 (相對於全螢幕)
        tile_centers = {}
        ...
        for r in range(self.BOARD_ROWS):
            for c in range(self.BOARD_COLS):
                ...
                if tile_img_cropped.size == 0:
                    board_matrix[r][c] = "裁剪錯誤" # 裁剪後變空，也視為錯誤
                    cv2.rectangle(debug_img_base,
                                  (self.GAME_BOARD_OFFSET_X + x, self.GAME_BOARD_OFFSET_Y + y),
                                  (self.GAME_BOARD_OFFSET_X + x + self.BOARD_TILE_WIDTH, self.GAME_BOARD_OFFSET_Y + y + self.BOARD_TILE_HEIGHT),
                                  (0, 0, 255), RECTANGLE_THICKNESS)
                ...
                if (0 <= pure_check_y_start < pure_check_y_end <= tile_img_cropped.shape[0] and
                    ...
                    # print(f"⚠️ 警告: 純色檢查區域裁剪無效，將使用整個方塊圖片 ({r},{c})。") # 注釋掉，避免過多輸出
                    pure_color_check_area = tile_img_cropped

                # 如果 pure_color_check_area 無效 (例如 size 為 0)，則不能進行後續判斷
                if pure_color_check_area is None or pure_color_check_area.size == 0:
                    board_matrix[r][c] = "裁剪錯誤" # 視為裁剪錯誤
                    cv2.rectangle(debug_img_base,
                                  (self.GAME_BOARD_OFFSET_X + x, self.GAME_BOARD_OFFSET_Y + y),
                                  (self.GAME_BOARD_OFFSET_X + x + self.BOARD_TILE_WIDTH, self.GAME_BOARD_OFFSET_Y + y + self.BOARD_TILE_HEIGHT),
                                  (0, 0, 255), RECTANGLE_THICKNESS)
                ...
                if is_empty_tile:
                    ...
                    cv2.putText(debug_img_base, "E", (self.GAME_BOARD_OFFSET_X + x + (self.BOARD_TILE_WIDTH - cv2.getTextSize("E", cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, TEXT_THICKNESS)[0][0]) // 2,
                                                    self.GAME_BOARD_OFFSET_Y + y + (cv2.getTextSize("E", cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, TEXT_THICKNESS)[0][1] + 10)),
    ...
    def is_passable(self, board, r, c, p1_original, p2_original):
        """
        檢查 (r, c) 座標是否可通行。
        可通行意味著它是空方塊，或者是當前正在檢查的 p1_original 或 p2_original 方塊。
        此函數也考慮了棋盤外部的無限空白區域。
        """
        ...
        if current_coords == p1_original or current_coords == p2_original:
            return True

        # 在棋盤內部，只有 "empty" 方塊才可通行
        return board[r][c] == "empty"

    def find_path_with_bends(self, board, p_start, p_end, max_bends: int, p1_original, p2_original) -> bool:
        """
        使用 BFS 搜尋從 p_start 到 p_end 的路徑，最多允許 max_bends 個轉角。
        p1_original 和 p2_original 是原始的消除對象，用於判斷可通行性。
        """

        # 狀態: (r, c, bends_count, last_direction)
        # last_direction: 0=None (起點), 1=Up, 2=Down, 3=Left, 4=Right

        # 佇列用於 BFS: (r, c, bends, last_dir)
        queue = deque([(p_start[0], p_start[1], 0, 0)])

        # 紀錄已訪問狀態: (r, c, bends, last_dir) -> True
        # 包含 bends 和 last_dir 是關鍵，確保能夠處理不同路徑和轉角數到達同一點的情況，避免無限循環。
        visited = set()
        visited.add((p_start[0], p_start[1], 0, 0)) # Initial state (r, c, bends, last_dir)


        # 定義移動方向
        # (dr, dc, direction_code)
        ...
        while queue:
            ...
            # 如果找到終點，檢查是否在允許的轉角數內
            if (r, c) == p_end:
                # 這裡不需要額外的 bends 檢查，因為我們已經在添加元素時篩選了 new_bends > max_bends
                return True

            # 遍歷所有可能的移動方向
            for dr, dc, current_dir in directions:
                nr, nc = r + dr, c + dc

                # 計算新轉角數
                new_bends = bends
                if last_dir != 0 and current_dir != last_dir: # 如果不是起點且方向改變
                    new_bends += 1
                ...
                # 檢查新位置是否可通行
                if not self.is_passable(board, nr, nc, p1_original, p2_original):
                    continue

                # 確保不重複訪問相同 (位置, 轉角數, 方向) 的狀態
                if (nr, nc, new_bends, current_dir) not in visited:
                    visited.add((nr, nc, new_bends, current_dir))
                    queue.append((nr, nc, new_bends, current_dir))
    ...
    def can_eliminate(self, board, p1, p2):
        ...
        # 注意：這裡的 "empty" 是指真正的空位，或者被 persisted_e_board 標記為 "E" 而邏輯上轉換為 "empty" 的位置
        ...
        # 現在使用 BFS 函數來判斷路徑
        # 最多允許 2 個轉角 (即 0彎, 1彎, 2彎)
        return self.find_path_with_bends(board, p1, p2, 2, p1, p2)
    ...
    while True:
        if running:
            try:
                ...
                for r1 in range(BOARD_ROWS):
                    for c1 in range(BOARD_COLS):
                        ...
                        # 注意：只對 tile_centers 中存在的方塊進行檢查，這些是可點擊的實體方塊
                        if p1 not in tile_centers: # 如果該點沒有中心座標，說明它不可點擊（例如被識別為 empty, lock, unknown, error）
                            continue

                        tile1_type = board_to_process[r1][c1]
                        # 再次檢查 tile1_type 是否是可玩的
                        non_clickable_check_in_loop = ["empty", "lock1", "lock2", "lock3", "lock4", "未知", "裁剪錯誤", "空ROI"]
                        if tile1_type in non_clickable_check_in_loop:
                            continue
                        ...
                        for r2 in range(BOARD_ROWS):
                            for c2 in range(BOARD_COLS):
                                p2 = (r2, c2)
                                if p1 == p2 or p2 not in tile_centers: # 跳過自身或不可點擊的對象
                                    continue
                                ...
                                if tile1_type == board_to_process[r2][c2]:
                                    # 檢查這個對子是否真的可以消除
                ...
                if eliminable_pairs:
                    ...
                    for p1, p2 in eliminable_pairs:
                        # 獲取實際點擊座標
                        click_x1, click_y1 = tile_centers[p1]
                        click_x2, click_y2 = tile_centers[p2]

                        print(f"✅ 消除對子: {board_to_process[p1[0]][p1[1]]} 在 {p1} 和 {p2}")

                        # 轉換為相對於 Debug 視窗的座標進行繪圖
                        # 注意：這裡的繪圖座標需要是相對於 img_game_window 的，而不是全螢幕的
                        # 因此需要減去 game_window_x, game_window_y
                        draw_x1_rel = click_x1 - game_window_x
                        draw_y1_rel = click_y1 - game_window_y
                        draw_x2_rel = click_x2 - game_window_x
                        draw_y2_rel = click_y2 - game_window_y

                        cv2.circle(display_img, (draw_x1_rel, draw_y1_rel), 10, LINE_COLOR, -1)
                        cv2.circle(display_img, (draw_x2_rel, draw_y2_rel), 10, LINE_COLOR, -1)
                        cv2.line(display_img, (draw_x1_rel, draw_y1_rel), (draw_x2_rel, draw_y2_rel), LINE_COLOR, LINE_THICKNESS)
                ...
                last_display_img = display_img.copy() # 保存顯示過的圖像，以在暫停時顯示
            ...
            if last_display_img is not None:
                # 暫停時，顯示最後一幀圖像，並在上面標註“暫停”
                display_img_copy = last_display_img.copy()
                text_pause = "Bot Paused (Press F3 to Start)"
                (text_width, text_height), baseline = cv2.getTextSize(text_pause, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
                # 計算文本位置，使其居中
                text_x = (display_img_copy.shape[1] - text_width) // 2
                text_y = display_img_copy.shape[0] // 2
                cv2.putText(display_img_copy, text_pause, (text_x, text_y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2) # 黃色字體
                display_img_resized = cv2.resize(display_img_copy, (display_width, display_height))
                cv2.imshow(display_window_name, display_img_resized)
            else:
                # 如果還沒有任何圖像可顯示，則顯示一個黑色畫面
